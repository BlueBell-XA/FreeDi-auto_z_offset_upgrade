...
class AutoZOffsetCommandHelper(probe.ProbeCommandHelper):
    def __init__(self, config, mcu_probe, query_endstop=None):
        ...
        self.bed_center_x = config.getfloat("bed_center_x", 120.0)
        self.bed_center_y = config.getfloat("bed_center_y", 120.0)
        self.invert_z = config.getboolean("invert_z", default=True)
    ...
    def _move_to_center(self, gcmd):
        ...
        target = [self.bed_center_x, self.bed_center_y, max(z, self.probe_hop), e]
        self._move(target, params["lift_speed"])

    def lift_probe(self, gcmd):
        toolhead = self.printer.lookup_object("toolhead")
        params = self.mcu_probe.get_probe_params(gcmd)
        x, y, z, e = toolhead.get_position()
        target = [x, y, z + self.probe_hop, e]
        self._move(target, params["lift_speed"])
    ...
    def cmd_AUTO_Z_PROBE(self, gcmd):
        ...
        self.last_probe_position = (pos[0], pos[1], pos[2])
    ...
    def cmd_AUTO_Z_HOME_Z(self, gcmd):
        ...
        toolhead.set_position(
            [x, y, self.z_offset, e], homing_axes=(0, 1, 2)
        )
        self.lift_probe(gcmd)
    ...
    def cmd_AUTO_Z_MEASURE_OFFSET(self, gcmd):
        ...
        x, y, z, e = toolhead.get_position()
        target = [
            self.bed_center_x - main_probe.probe_offsets.x_offset,
            self.bed_center_y - main_probe.probe_offsets.y_offset,
            z,
            e
        ]
        self._move(target, params["lift_speed"])
